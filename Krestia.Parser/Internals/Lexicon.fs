...
type VerbInfo =
   { FullForm: string option
     ...
     ArgumentRemarks: string option list option }
...
type DictionaryBuilder() =
   ...
   member _.AddModifier(lexicon: Lexicon, spelling, meaning, gloss, ?roots, ?remarks, ?attachmentRemarks) =
      let modifier =
         { Spelling = spelling
           Meaning = meaning
           Gloss = gloss
           Roots = defaultArg roots []
           Remarks = remarks
           WordTypeInfo = Modifier { AttachmentRemarks = attachmentRemarks } }
